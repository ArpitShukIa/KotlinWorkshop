...
fun readResponses(): List<Response> {
    ...
    for (i in 1 until inputString.size){
        ...
        responses.add(Response(eachline[1], eachline[2].toInt(), eachline[3], eachline[4]=="yes"))
    }
    return responses
}

fun processResponses(responses: List<Response>): List<List<Response>> {
    return responses.filter { !it.covidPositive }.sortedWith(compareBy({ it.branch }, { it.name })).groupBy { it.branch }.values.chunked(3)
        .map { it.flatten() }
}
    ...
    File("src/main/kotlin/assignment/batches.txt").bufferedWriter().use { out ->
        for((index, value) in batches.withIndex()) {
            ...
            for((studIndex, student) in value.withIndex()){
                out.write("${studIndex+1}\t\t\t${student.name}\t\t\t${student.branch}\t\t\t${student.rollNumber}")
                out.newLine()
            }
            out.newLine()
        }
